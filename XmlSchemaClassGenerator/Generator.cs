        ...
        public void Generate(XmlSchemaSet set)
        {
            ...
            if (_configuration.OutputType == Enums.OutputType.CSharp)
            {
                ...
                foreach (var ns in namespaces)
                {
                    if (Version != null)
                    {
                        ns.Comments.Add(new CodeCommentStatement($"This C# code was generated by {Version.Title} version {Version.Version}."));
                    }
            ...
            {
                var writer = _configuration.OutputWriter ?? new SQL.Output(OutputFolder ?? ".") { Configuration = _configuration };
                //foreach (var ns in namespaces)
                //{
                //    if (Version != null)
                //    {
                //        ns.Comments.Add(new CodeCommentStatement($"This SQL code was generated by {Version.Title} version {Version.Version}."));
                //    }
                //    writer.Write(ns);
                //}

                //Writer outside namespace iterator, allows for key linking between separate XSD/namespaces
                writer.Write(namespaces.ToList());
            }
        }
    }
}